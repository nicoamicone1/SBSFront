...
const EditCreate:React.FC<Props> = ({product,create}) => {
    ...
    const saveChanges=()=>{
        ...
        if(dummy.name===""||dummy.price===0||dummy.description===""||dummy.image_url===defaultImg){
            return setPleaseComplete(true)
        }
        
        if(create){
            axios.post(`${api}/products`,dummy)
            .then((res)=>{
                if(res.status===200){
                    swal.fire({
    ...
    return (
        <div>
            ...
            <IconButton onClick={handleClickOpen} sx={{background:color.goldgradiant,borderRadius:2}}>
                <AddIcon/>
                        ...
                        <Box sx={{width:"80%",mt:1}}>
                            {pleaseComplete&&
                                <Box sx={{display:"flex",justifyContent:"center"}}>
                                    <Typography color="error">Por favor completar todos los campos</Typography>   
                                </Box>
                            }
